...
function buildSystemPrompt(attachments?: string): string {
  ...
  return [
    ...
    "- MANAGING TASKS: To mark/complete/finish a task, or to remove/dismiss/delete one, you MUST",
    "  call update_task_status (never create_task). First call list_tasks to get the real IDs —",
    "  the USER DATA block is a summary and may omit tasks, so NEVER conclude a task doesn't exist",
    "  from it. To act on every task at once, call update_task_status with id='all'. Do NOT create",
    "  a new 'Complete X' task to represent finishing task X.",
